...
def yolo_head(yolo_out, anchors, num_classes, input_shape):
    """Convert final layer features to bounding box parameters."""
    num_anchors = len(anchors)
    # Reshape to batch, height, width, num_anchors, box_params.
    anchors_tensor = K.reshape(K.constant(anchors), [1, 1, 1, num_anchors, 2])

    grid_shape = K.shape(yolo_out)[1:3] # height, width
    grid_y = K.tile(K.reshape(K.arange(0, stop=grid_shape[0]), [-1, 1, 1, 1]),[1, grid_shape[1], 1, 1])
    grid_x = K.tile(K.reshape(K.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]),[grid_shape[0], 1, 1, 1])
    grid   = K.concatenate([grid_x, grid_y])
    grid   = K.cast(grid, K.dtype(yolo_out))

    yolo_out  = K.reshape(yolo_out, [-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 5])

    box_xy = K.sigmoid(yolo_out[..., :2])
    box_wh = K.exp(yolo_out[..., 2:4])
    box_confidence = K.sigmoid(yolo_out[..., 4:5])
    box_class_probs = K.sigmoid(yolo_out[..., 5:])

    # Adjust preditions to each spatial grid point and anchor size.
    box_xy = (box_xy + grid) / K.cast(grid_shape[::-1], K.dtype(yolo_out))
    box_wh = box_wh * anchors_tensor / K.cast(input_shape[::-1], K.dtype(yolo_out))
